...
(defconst hu-font-lock-keywords
  (list
   ...
   '("^[[:space:]]*#\\(?:b\\(?:ankcount\\|egin\\|ranchflag\\(?:clear\\|\\(?:re\\)?set\\)\\|s\\)\\|c\\(?:all\\|learflag\\)\\|endroutine\\|g\\(?:lobal\\(?:flag\\)?\\|roup\\)\\|in\\(?:clude\\|directcallers?\\)\\|local\\(?:flag\\)?\\|param\\(?:flag\\)?\\|r\\(?:e\\(?:gion\\|setflag\\|turn\\)\\|outine\\)\\|s\\(?:etflag\\|lot\\|ublocal\\(?:flag\\)?\\)\\)\\>" . font-lock-builtin-face)
   '("^[[:space:]]*#\\sw+" . font-lock-warning-face)
   '("#" . font-lock-constant-face)
   '("\\<\\(?:\\$[0-9A-Fa-f]+\\|[0-9]+\\)\\>" . font-lock-constant-face)
   '("\\<\\(?:A\\(?:DC\\|ND\\|SL\\)\\|B\\(?:C[CS]\\|EQ\\|IT\\|MI\\|NE\\|PL\\|R[AK]\\|V[CS]\\|B[RS][0-7]\\)\\|C\\(?:L[CDIV]\\|MP\\|P[XY]\\)\\|DE[ACXY]\\|EOR\\|IN[ACXY]\\|J\\(?:MP\\|SR\\)\\|L\\(?:D[AXY]\\|SR\\)\\|NOP\\|ORA\\|P\\(?:H[APXY]\\|L[APXY]\\)\\|R\\(?:O[LR]\\|T[IS]\\|MB[0-7]\\)\\|S\\(?:BC\\|E[CDI]\\|T[APXYZ]\\|MB[0-7]\\)\\|T\\(?:A[XY]\\|RB\\|S[BX]\\|[XY]A\\)\\|WAI\\)\\(?:\\.[WB]\\)?\\>" . font-lock-keyword-face)
